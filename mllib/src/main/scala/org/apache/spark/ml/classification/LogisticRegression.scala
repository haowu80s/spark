...
class LogisticRegression @Since("1.2.0") (
  ...
  /**
   * A interect interface with training method using local arrays
   */
  def localTrain(localData: Array[(Double, Double, Vector)]): LogisticRegressionModel = {

    // Instance(label, weight, features)
    val instances = localData.map(v => Instance(v._1, v._2, v._3))

    val summarizer = instances.aggregate(new MultivariateOnlineSummarizer)((c, i) => c.add(i.features), (c1, c2) => c1.merge(c2))
    val labelSummarizer = instances.aggregate(new MultiClassSummarizer)((c, i) => c.add(i.label, i.weight), (c1, c2) => c1.merge(c2))
    ...
    // summary is not provided for this method
  }
